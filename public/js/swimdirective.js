	...
	return {
		link: function ($scope, element, attrs) {
			$scope.$on('swimChart', function (event, dataset) {
				console.log('Wonder-twin powers, activate!')
				...
				function timeline(domElement, dataset) {
					...
					timeline.band = function (bandName, sizeFactor) {
						...
						band.h = ((data.items.length*30) - margin.top - margin.bottom) * (sizeFactor || 1);
						...
						var infoAppend = function(d) {
							$(document).ready(function() {
								$('#SwimChartTable').dataTable( {
									...
									"bRebuild": true,
									"aoColumns": [
										{ "sTitle": "Time", "mData":"time" },{ "sTitle": "Lan IP", "mData":"lan_ip" }
									]
								} );
								$('html').animate({
