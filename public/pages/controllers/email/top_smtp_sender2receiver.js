...
angular.module('mean.pages').controller('topSmtpSender2ReceiverController', ['$scope', '$stateParams', '$location', 'Global', '$rootScope', '$http', function ($scope, $stateParams, $location, Global, $rootScope, $http) {
	...
	if ($location.$$search.start && $location.$$search.end) {
		query = '/email/top_smtp_sender2receiver?start='+$location.$$search.start+'&end='+$location.$$search.end+'&from='+$location.$$search.from;
	} else {
		query = '/email/top_smtp_sender2receiver?&from='+$location.$$search.from;
	}
