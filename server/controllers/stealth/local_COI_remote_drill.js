	...
	return {
		render: function(req, res) {
			...
			if (req.query.ip && (permissions.indexOf(parseInt(req.session.passport.user.level)) !== -1)) {
				...
				async.parallel([
					// FISHEYE	
					function(callback) { // stealth_conn_ioc
						console.log(stealth_conn_ioc);
						new fisheye(stealth_conn_ioc, {database: database, pool:pool}, function(err,data, maxConn, maxIOC){
