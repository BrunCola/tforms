...
module.exports = function (sql1, sql2, sql3, sql4, sql5, sql6, conn, callback) {
	var result = {};
	...
	conn.pool.getConnection(function(err, connection) {
		...
		async.series([
			...
			function(callback){
				connection.query(sql5.query, sql5.insert, function(err, result) {
			...
			function(callback){
				connection.query(sql6.query, sql6.insert, function(err, result) {
					//this query returns the remote cleartext connections to and from the central IP
					if (err) {
						callback(err, null);
					} else {
