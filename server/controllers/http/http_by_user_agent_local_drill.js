...
module.exports = function(pool) {
    return {
        render: function(req, res) {
            ...
            if (req.query.lan_zone && req.query.lan_ip && req.query.user_agent) {
                var tables = [];
                var info = [];
                var table1 = {
                    query: 'SELECT ' +
                            ...
                            'WHERE '+ 
                                ...
                                'AND `lan_ip` = ? ' +
                                'AND `user_agent` = ?',
                    insert: [start, end, req.query.lan_zone, req.query.lan_ip, req.query.user_agent],
                    params: [
                        {
                            title: 'Time',
                            select: 'time'
                    ...
                    settings: {
                        sort: [[0, 'desc']],
                        div: 'table',
                        title: 'Common HTTP Connections between Domain and User Agent',
